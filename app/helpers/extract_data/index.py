...
async def redirects_row_data(row: dict, browser) -> List[Dict[str, Any]]:
    ...
    for site in websites:
        if site not in completed_brands:
            site_results.append({"site": site, "error": f"Dev Pending {site}"})
            continue  # Skip uncompleted brands cleanly
        ...
        try:
            ...
            # Log the error and save failure state so the response knows this site failed
